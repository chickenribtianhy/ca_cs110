...
int sum_vectorized(int n, int *a)
{
    ...
    for (int i = 0; i < n / 4 * 4; i += 4)
    {
        temp = _mm_add_epi32(temp, _mm_loadu_si128((__m128i *)(a + i)));
    }
    int tmp[4];
    int result;
    _mm_storeu_si128((__m128i *)tmp, temp);
...
int sum_vectorized_unrolled(int n, int *a)
{
    ...
    {
        temp = _mm_add_epi32(temp, _mm_loadu_si128((__m128i *)(a + i)));
        temp = _mm_add_epi32(temp, _mm_loadu_si128((__m128i *)(a + i + 4)));
        temp = _mm_add_epi32(temp, _mm_loadu_si128((__m128i *)(a + i + 8)));
        temp = _mm_add_epi32(temp, _mm_loadu_si128((__m128i *)(a + i + 12)));
    }
    int tmp[4];
    int result;
    _mm_storeu_si128((__m128i *)tmp, temp);
