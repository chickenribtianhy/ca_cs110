...
void transpose_blocking(int n, int blocksize, int *dst, int *src)
{
    ...
    for (int xx = 0; xx < n; xx += blocksize)
    {
        ...
        {
            for (int x = 0; x < blocksize && x < n - xx; x++)
            {
                for (int y = 0; y < blocksize && y < n - yy; y++)
                {
                    // dst[y + x * n] = src[x + y * n];
                    // dst[y + yy + x * n + xx * n] = src[0];
                    dst[y + yy + x * n + xx * n] = src[x + xx + y * n + yy * n];
                }
            }
        }
    }
    ...
    //         {
    //             // dst[y + x * n] = src[x + y * n];
    //             // dst[0] = src[x + y * n + yy * n];
    //             dst[y + yy + x * n] = src[x + y * n + yy * n];
    //         }
    //     }
    // }
}
