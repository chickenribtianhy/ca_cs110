...
static void ring_buffer_grow(ring_buffer *buffer)
{
    ...
    int new_capacity;
    int *new_content;
    printf("%ld, %ld, %ld\n", buffer->capacity, buffer->read_pos, buffer->write_pos);
...
bool ring_buffer_write(ring_buffer *buffer, const int data)
{
    ...
    /* judge whether the buffer is full, if it is, just expand it, make sure it's not full every time we want to write in */
    if (buffer->write_pos == buffer->read_pos) /* the buffer is full when tail meats head */
